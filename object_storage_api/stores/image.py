...
class ImageStore:
    ...
    def get_object_key(self, image_id: str, image_metadata: ImagePostMetadataSchema) -> str:
        """
        Returns the object key for an image not yet uploaded.
        ...
        """
        return f"images/{image_metadata.entity_id}/{image_id}"

    def upload(self, image_in: ImageIn, upload_file: UploadFile) -> str:
        """
        Uploads a given image to object storage.

        :param image_metadata: Metadata of the image to be uploaded.
        :param image_in: `ImageIn` model of the image.
        """
        logger.info("Uploading image file with object key: %s to the object store", image_in.object_key)
        s3_client.upload_fileobj(
            upload_file.file,
            Bucket=object_storage_config.bucket_name.get_secret_value(),
            Key=image_in.object_key,
